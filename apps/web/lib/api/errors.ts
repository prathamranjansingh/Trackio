...
export class TrackioApiError extends Error {
...
const docErrorUrl = "https://google.com/";
...
export function handleApiError(error: any): ErrorResponse & { status: number } {
  ...
  // TrackioApiError errors
  if (error instanceof TrackioApiError) {
    return {
      error: {
        code: error.code,
